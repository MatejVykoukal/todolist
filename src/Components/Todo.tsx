...
const Todo: React.FC<Props> = ({ id, completed, name }) => {
  const todos = useSelector((state: { todos: { todos: TodoModel[] } }) => state.todos.todos);
  const todo = useSelector((state: { todos: TodoModel[] }) => todos.find((el) => el.id === id)!);
  ...
  return (
    <div className="todo">
      <li className={completed ? "todo-item completed" : "todo-item"}>{name}</li>
